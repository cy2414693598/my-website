...
> 主线项目：**个人成长工作站**——从纯静态页面起步，一路演进为带用户系统的全栈个人成长平台（v1.1 定位，见 docs/REQUIREMENTS.md 决策 D9）。
> 选它的原因：需求直观、天然覆盖全栈所有环节（前端、后端、数据库、部署、运维）；容器先行——引擎（阶段 3~7）一次造好，内容形态（文章 → 训练 → 作品集 → 相册）在 V2 渐次生长。
...
5. **写 build log**：每次学到什么、踩了什么坑，随手记几行。这既是复习材料，也是网站文章的第一批素材。
...
- [x] 实现文章的增删改查（CRUD）REST 接口，数据先存内存或 JSON 文件（下一阶段换数据库）（2026-08-27 完成：Controller→Service 全通，MockMvc 3 测试全绿，curl 七场景实测全过；踩坑三连入册：兜底异常需专用 404 处理、JSON 数字反序列化 Integer/Long 陷阱、Windows 终端 curl 内联中文会编码损坏须用 @file 发送）
...
- [ ] 部署方案决策（三选一或混合，届时正式入 REQUIREMENTS 决策表）：① 轻量云服务器（新人机常低至约 99 元/年）② 本地机器/树莓派 homelab（需解决公网可达：Cloudflare Tunnel 首选 / IPv6 / frp）③ 混合架构：博客前台留 GitHub Pages + 后端与私人服务（训练营/后台）本地跑——私人用途的服务永远适合本地，公开服务才需要托管
- [ ] Linux 基础操作、Nginx 反向代理、域名解析（本地方案同样适用：虚拟机或树莓派装 Linux 全套照学）
- [ ] 用 Docker 把后端打包部署（推荐，可选项但值得学）
- [ ] 配 HTTPS 证书（Let's Encrypt 免费；Cloudflare Tunnel 方案自带）
- [ ] 决策点：境内服务器需 ICP 备案（免费、2~4 周）；不想备案选境外节点或纯本地方案
- [ ] 数据与运维兜底：MySQL 定时备份（mysqldump + 异地存储）、关键接口日志、健康检查端点

**完成标准**：全栈网站跑在自己的服务器上，域名 + https 访问，重启后服务自动恢复。

## 阶段 8+：迭代循环（不设限）

每个功能 = 一次小规模学习，一次只加一个。优先级与 docs/REQUIREMENTS.md §4/§9 保持同步：

- 评论系统（数据库关联、防刷组合，见 D6）
- **每日训练营**（AI 阶梯第 1 项，见 D7/D8：演讲台 / 八股特训 / 思维体操）
- 可见性控制（私密内容登录可见，见 D9——"先打磨不宣传"策略的技术支撑）
- 图床 + 作品集页（剪辑/摄影内容形态；图床优先级因创作需求上调）
- 思维导图页（图片起步，进阶 markmap 渲染）与旅行相册（内容形态扩展，见 REQUIREMENTS §8 成长轴）
- 访问统计仪表盘（图表库、聚合查询）
- 性能优化（缓存、懒加载、Lighthouse 跑分）、标签分类 / RSS / 站内搜索
- ~~自动化部署 CI/CD~~ → **已在阶段 2 提前完成**（GitHub Actions，push 即上线）

## 预算汇总

| 项目 | 金额 |
|---|---|
| 阶段 0~6 全程 | 0 元（免费托管 + 本地 MySQL 社区版） |
| 域名（可选，建议买） | 约 60~80 元/年 |
| 阶段 7 起（三选一，见阶段 7 决策项） | 本地 homelab **0 元** / 轻量云新人机约 **99 元/年**（常规价约 50~100 元/月）/ 混合架构 |
| LLM API（AI 阶梯启用后） | DeepSeek 等，个人用量一年约几十元 |
